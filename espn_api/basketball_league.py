...
from .base_settings import BasketballSettings
from .base_pick import BasketballPick
from .utils.logger import Logger
from .requests.espn_requests import EspnFantasyRequests

class BasketballLeague(ABC):
    '''Creates a League instance for Public/Private ESPN league'''
    def __init__(self, league_id: int, year: int, espn_s2=None, swid=None, debug=False):
        ...
        self.espn_request = EspnFantasyRequests(year=year, league_id=league_id, cookies=cookies, logger=self.logger)

    def __repr__(self):
        return 'BasketballLeague(%s, %s)' % (self.league_id, self.year, )

    def _fetch_league(self, SettingsClass = BasketballSettings):
    ...
    def _fetch_draft(self):
        ...
        for pick in picks:
            ...
            self.draft.append(BasketballPick(team, playerId, playerName, round_num, round_pick, bid_amount, keeper_status, nominatingTeam))
